...
// Modal simplifiée + couleurs alignées
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
        <motion.div
          ...
          className="w-full max-w-5xl max-h-[95vh] sm:max-h-[90vh] rounded-2xl sm:rounded-3xl bg-slate-950 border border-slate-800/60 shadow-2xl overflow-hidden flex flex-col lg:flex-row pointer-events-auto"
        >
          {/* Image */}
          <div className="flex-1 bg-slate-900/50 p-4 sm:p-8 flex items-center justify-center relative min-h-[300px] sm:min-h-auto">
            <motion.img
              key={activeVariantId}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            {product.variants.length > 1 && (
              <div className="absolute bottom-2 sm:bottom-6 left-1/2 -translate-x-1/2 flex gap-2 sm:gap-4 bg-black/60 backdrop-blur-lg p-2 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-700/50">
          ...
          {/* Infos */}
          <div className="flex-1 p-6 sm:p-10 flex flex-col justify-between border-t lg:border-t-0 lg:border-l border-slate-800/60">
            <button onClick={onClose} className="absolute top-4 sm:top-6 right-4 sm:right-6 p-2 sm:p-3 rounded-full bg-slate-900/60 border border-slate-700/50">
              <X className="w-5 h-5 sm:w-6 sm:h-6 text-slate-300" />
            </button>

            <div className="space-y-6 sm:space-y-8 overflow-y-auto max-h-[calc(95vh-160px)] sm:max-h-[calc(90vh-160px)]">
              <div>
                <span className="inline-block px-3 sm:px-4 py-1 sm:py-1.5 rounded-full text-xs sm:text-sm font-medium"
                  style={{
                ...
                <h2 className="mt-2 sm:mt-4 text-2xl sm:text-4xl md:text-5xl font-bold text-white leading-tight">
                  {activeVariant?.title}
                </h2>
              </div>

              <p className="text-sm sm:text-lg text-slate-300 leading-relaxed">{activeVariant?.description}</p>
            ...
            <div className="mt-10 space-y-4">
              <button className="w-full bg-gradient-to-r from-teal-600 to-cyan-600 text-white font-medium py-4 rounded-xl flex items-center justify-center gap-3 hover:brightness-110 transition-all">
                <ShoppingCart className="w-5 h-5" /> Demander un devis
              </button>
              <button className="w-full border border-slate-600 hover:border-teal-500 text-white font-medium py-4 rounded-xl flex items-center justify-center gap-3 transition-all">
                <MessageSquare className="w-5 h-5" /> Contacter un expert
              </button>
            </div>
          </div>
        </motion.div>
